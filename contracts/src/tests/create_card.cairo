...
use tsubasa::components::{Game, Roles, RolesU8, Card};
...
fn test_create_card() {
    ...
    let mut create_card_calldata = array![1, 22, 17, 10, 3, 1];
    world.execute('create_card_system', create_card_calldata);

    let mut create_card_calldata_player2 = array![2, 10, 15, 15, 1, 0];
    ...
    assert(card_player1.role == RolesU8::Attacker, 'role is wrong');
    ...
    assert(card_player2.role == RolesU8::Defender, 'role is wrong');
